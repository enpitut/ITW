<?php
  ...
  try{
    ...
    if($dbh == null){
      ...
      $check_sql = 'SELECT * FROM friends WHERE user_id = :user_id';
      ...
      fclose($fp);
      if($rc == 0){
        $insert_sql = 'INSERT INTO positions (user_id, friend_id) values (:user_id, :friend_id)';
        $stmt = $dbh->prepare($insert_sql);
        $stmt->bindValue(':friend_id', $friend_id, PDO::PARAM_INT);
        $stmt->bindValue(':user_id', $user_id, PDO::PARAM_INT);
        $stmt->execute();
      }else{
        $update_sql = 'UPDATE friends SET friend_id = :friend_id WHERE user_id=:user_id';
